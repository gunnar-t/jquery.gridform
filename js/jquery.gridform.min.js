...
return false;if(enable===true){$(cellSelector).find("input").removeAttr("disabled");}else if(enable===false){$(cellSelector).find("input").attr("disabled","disabled");}}});gridform.types.headline=$.extend({},mastertype,{labelAllowed:false,containsData:false,render:function(data,cellSelector,parent){var html='<div class="headline">'+data.label+'</div>';return html;}});gridform.types.separator=$.extend({},mastertype,{labelAllowed:false,containsData:false,render:function(data,cellSelector,parent){var html='<div class="separator"></div>';return html;}});gridform.addType=function(type,object){gridform.types[type]=$.extend({},mastertype,object);};gridform.setDefaults=function(settings){gridform.form.prototype.defaultSettings=$.extend(gridform.form.prototype.defaultSettings,settings);};$.fn.gridform=function(settings){if(gridform.forms[settings.name]!==undefined){console.error("A form with the name '"+settings.name+"' already exists!");}
var obj=new gridform.form(settings);if(obj===false)
return false;if($(this).length===1)
obj.render(this);gridform.forms[settings.name]=obj;return obj;};}
(jQuery));