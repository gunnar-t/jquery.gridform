var gridform=gridform||{};(function($){gridform.forms={};gridform.types={};gridform.form=function(settings){return this.init(settings);};gridform.form.prototype={settings:null,fieldsById:{},renderedAtTarget:null,rendered:null,fieldCount:null,init:function(settings){this.settings=$.extend({debug:false,mode:'edit',labelType:'inline',labelAlign:'right',showTooltipInstantly:false,successIsGreen:false,useFontAwesome:false,icon_success:'glyphicon glyphicon-ok',icon_error:'glyphicon glyphicon-remove',icon_warning:'glyphicon glyphicon-warning-sign',icon_waiting:'glyphicon glyphicon-refresh',language:{'mandatoryField':'This is a mandatory field','loading':'Loading ...','selectPlaceholder':'-',},dimensions:{},},settings);if(this.settings.name===undefined){console.error("A 'name' for the gridform is needed!");this.settings={};return false;}
return this;},render:function(target,debugOptions){this.fieldCount=0;this.rendered=0;this.fieldsById={};if(typeof target==="object"){if(target.selector===undefined)
return false;else
...
return false;$(this.renderedAtTarget).html("");this.rendered+=1;this.renderedAtTarget=this.settings.target;var html=this.__buildTable(debugOptions);$(this.settings.target).html(html);this.__buildContents(debugOptions);this.setData(this.settings.record);$(gridform.forms[this.settings.name]).trigger("rendered",[{'rendered':this.rendered}]);return this;},__buildTable:function(debugOptions){this.cols=0;this.rows=0;var fields={};for(var x in this.settings.fields){var splitting=x.split("_");var r=parseInt(splitting[0],10);var c=parseInt(splitting[1],10);if(isNaN(c)||isNaN(r)){console.error("The field "+this.settings.fields[x].id+" is missing correct row and col parameter!");continue;}
this.cols=(c>this.cols)?c:this.cols;this.rows=(r>this.rows)?r:this.rows;this.settings.fields[x].row=r;this.settings.fields[x].col=c;if(this.fieldsById[this.settings.fields[x].id]!==undefined){console.error("The id '"+this.settings.fields[x].id+"' is already taken, the field is deleted from the grid!");delete this.settings.fields[x];}else{this.fieldsById[this.settings.fields[x].id]=this.settings.fields[x];if(gridform.types[this.settings.fields[x].type]!==undefined&&gridform.types[this.settings.fields[x].type].containsData===true){this.fieldCount++;}}}
...
for(var x in this.settings.fields){var type=this.settings.fields[x].type;var field=this.settings.fields[x];var cellSelectorLabel=this.settings.target+' td[data-id='+x+'].gridform_label';var cellSelectorContent=this.settings.target+' td[data-id='+x+'].gridform_content';if(gridform.types[type]!==undefined&&typeof gridform.types[type]['setLabel']==="function"){var html=gridform.types[type]['setLabel'](field,cellSelectorLabel,this);if(typeof html==="string"){$(cellSelectorLabel).html(html);}}else{console.error("No setLabel function for type "+type);}
...
return false;if(enable===true){$(cellSelector).find("input").removeAttr("disabled");$(cellSelector).find("label").removeClass("disabled");}else if(enable===false){$(cellSelector).find("input").attr("disabled","disabled");$(cellSelector).find("label").addClass("disabled");}}});gridform.types.boolean=$.extend({},mastertype,{render:function(data,cellSelector,parent){var width=(data.width!==undefined)?'width:'+data.width:'';var disabled=(parent.settings.mode==="edit")?'':'disabled';var fontA=(parent.settings.useFontAwesome===true)?'fontA':'';var html='<div class="form-group">';var uniqueId=parent.settings.name+'_'+data.id;html+='<div class="checkbox '+fontA+'">';html+='<input type="checkbox" class="'+fontA+'" id="'+uniqueId+'" '+disabled+' name="" value="1"> <label for="'+uniqueId+'">&nbsp;</label>';html+='</div>';html+='</div>';return html;},set:function(data,value,cellSelector,parent){$(cellSelector).find("input").prop("checked",false);if(value===true){$(cellSelector).find("input[name='"+value+"']").prop("checked",true);}else{$(cellSelector).find("input[name='"+value+"']").prop("checked",false);}},get:function(data,cellSelector){var boxes=$(cellSelector).find("input:checked");if(boxes.length>0){return true;}else{return false;}},getFieldNode:function(field,cellSelector){return $(cellSelector).find("input");},flush:function(cellSelector,parent){$(cellSelector).find("input").prop("checked",false);},enable:function(data,enable,cellSelector){if(data.readonly===true)
return false;if(enable===true){$(cellSelector).find("input").removeAttr("disabled");}else if(enable===false){$(cellSelector).find("input").attr("disabled","disabled");}}});gridform.types.headline=$.extend({},mastertype,{labelAllowed:false,containsData:false,render:function(data,cellSelector,parent){var html='<div class="headline">'+data.label+'</div>';return html;}});gridform.types.separator=$.extend({},mastertype,{labelAllowed:false,containsData:false,render:function(data,cellSelector,parent){var html='<div class="separator"></div>';return html;}});gridform.addType=function(type,object){gridform.types[type]=$.extend({},mastertype,object);};$.fn.gridform=function(settings){if(gridform.forms[settings.name]!==undefined){console.error("A form with the name '"+settings.name+"' already exists!");}
var obj=new gridform.form(settings);if(obj===false)
return false;if($(this).length===1)
obj.render(this);gridform.forms[settings.name]=obj;return obj;};}(jQuery));